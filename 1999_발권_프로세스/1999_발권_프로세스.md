...
발권 프로세스는 VOF 시스템에서 자동 확정 가능 여부를 판단하여, 가능하면 자동으로 확정 및 발권을 진행하고, 불가능하면 운영팀이 수동으로 미확정 사유를 검토한 후 다양한 케이스별 조건(항공사 상이, 금액 상이, 스탑오버, 증빙서류)을 병렬로 확인하여 처리하는 프로세스입니다.
...
flowchart TD
    ...
    Cond1{온라인: 출국 항공사와 귀국 항공사 동일 여부}:::online
...
### 1. 자동 확정 단계
- VOF 시스템이 자동 확정 가능 여부를 판단합니다.
- 가능한 경우: 자동으로 확정 처리하고 결제 및 발권까지 진행하여 프로세스를 종료합니다.
- 불가능한 경우: 수동 처리 단계로 진행합니다.

### 2. 수동 검토 단계
- 운영팀이 VOF에서 미확정 사유를 확인하고 코멘트를 검토합니다.
- 검토 후 4가지 케이스를 병렬로 확인합니다.

### 3. 병렬 검토 케이스

#### 케이스1: 항공사 상이 검토
- 출국 항공사와 귀국 항공사가 다른 경우
- 운영팀이 VOF에서 여행 일정 확인 → GDS에서 확정 가능 예약 조회 → VOF에서 예약 확정 또는 취소 처리

#### 케이스2: 금액 상이 검토
- 예약 금액과 발권 금액이 다른 경우
- 운영팀이 VOF에서 TL 연장 → ZD 이메일로 여행자에게 안내 발송 → 여행자 컨펌 회신 → GDS에서 최종 금액 재확인 → VOF에서 확정 처리

#### 케이스3: 스탑오버 검토
- 스탑오버가 2회 이상인 경우
- 운영팀이 GDS에서 운임 규정 확인 → 스탑오버 가능 여부 확인 → VOF에서 필요 시 사용 가능한 일정 재확인 → VOF에서 확정 또는 취소 처리

#### 케이스4: 증빙서류 검토
- 증빙서류가 필요한 경우
- 운영팀이 ZD 이메일로 서류 제출 요청 발송 → 여행자가 마이트립 내여행에 서류 업로드 → 운영팀이 VOF에서 서류 확인 → 확정 진행

### 4. 프로세스 종료
- 모든 해당 케이스의 처리가 완료되면 발권 프로세스가 종료됩니다.
