...
VOF 시스템에서 자동 확정 가능 여부를 판단하고, 불가 시 운영팀이 수동으로 4가지 케이스(항공사 상이, 금액 상이, 스탑오버, 증빙서류)를 병렬로 검토하여 발권을 진행하는 프로세스입니다.
...
- 가능하면 자동으로 확정 처리 후 결제 및 발권을 진행하고 종료합니다.
- 불가하면 운영팀의 수동 처리로 넘어갑니다.

### 2. 수동 검토 단계
- 운영팀이 VOF에서 미확정 사유를 확인하고 코멘트를 검토합니다.
- 이후 4가지 케이스를 병렬로 확인합니다.

### 3. 케이스별 처리

#### 케이스1: 항공사 상이
- 출국/귀국 항공사가 다른 경우
- VOF에서 여행 일정 확인 → GDS에서 확정 가능 예약 조회 → VOF에서 확정/취소 처리

#### 케이스2: 금액 상이
- 예약 금액과 발권 금액이 다른 경우
- VOF에서 TL 연장 → ZD이메일로 안내 → 여행자 컨펌 → GDS에서 최종 금액 확인 → VOF에서 확정

#### 케이스3: 스탑오버 2회 이상
- 스탑오버가 2회 이상인 경우
- GDS에서 운임 규정 및 스탑오버 가능 여부 확인 → VOF에서 일정 재확인(필요 시) → 확정/취소 처리

#### 케이스4: 증빙서류 필요
- 증빙서류 제출이 필요한 경우
- ZD이메일로 서류 요청 → 여행자가 마이트립에서 업로드 → VOF에서 서류 확인 후 확정
