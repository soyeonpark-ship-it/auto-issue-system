...
VOF에서 자동 확정 가능 여부를 판단하여 자동 또는 수동으로 발권을 처리하는 프로세스입니다. 수동 처리 시 항공사 상이, 금액 상이, 스탑오버, 증빙서류 등 4가지 조건을 병렬로 확인하여 처리합니다.
...
- 자동 확정이 가능하면 자동으로 확정 처리 후 결제 및 발권까지 진행하고 프로세스가 종료됩니다
- 자동 확정이 불가하면 수동 처리로 전환됩니다

### 2. 수동 처리 초기 단계
- 운영팀이 VOF에서 미확정 사유를 확인합니다
- 코멘트를 검토하여 어떤 조건에 해당하는지 파악합니다
...
#### 케이스 2: 예약/발권 금액 상이
- 예약 금액과 발권 금액이 다른 경우
- 운영팀이 VOF에서 TL 연장 → ZD 이메일로 금액 상이 안내 발송 → 여행자 컨펌 회신 대기 → GDS에서 최종 금액 재확인 → VOF에서 확정 처리

#### 케이스 3: 스탑오버 2회 이상
- 스탑오버가 2회 이상인 경우
- 운영팀이 GDS에서 운임 규정 및 스탑오버 가능 여부 확인 → 필요 시 VOF에서 사용 가능한 일정 재확인 → VOF에서 예약 확정 또는 취소 처리

#### 케이스 4: 증빙서류 필요
- 증빙서류 제출이 필요한 경우
- 운영팀이 ZD 이메일로 서류 제출 요청 발송 → 여행자가 마이트립 내여행에 서류 업로드 → 운영팀이 VOF에서 서류 확인 후 확정 진행
