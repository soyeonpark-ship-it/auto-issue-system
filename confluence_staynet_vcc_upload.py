...
TARGET_PAGE_ID = "1192395065"
CONFLUENCE_EMAIL = os.getenv("CONFLUENCE_EMAIL")
CONFLUENCE_API_TOKEN = os.getenv("CONFLUENCE_API_TOKEN")
IMAGE_FOLDER = "mermaid_images"
IMAGE_FILENAME = "staynet_hotel_vcc.png"
...
def generate_confluence_html_with_image(image_filename):
    ...
    <h2>프로세스 흐름</h2>
    <p>요금표 작성 → 계정 생성 → GPID 매핑 → 파트너 페이지 접속 → 요금 입력 → 작업 완료 회신 → 담당자 지정 요청 → 담당자 지정 → 담당자 전달 → 판매시작 요청 → 온세일 처리 → 판매 시작</p>
    """
    return html
...
def main():
    ...
    print("=" * 60)
    print(f"Confluence Wiki 생성 - 스테이넷 직계약 호텔")
    print("=" * 60)
